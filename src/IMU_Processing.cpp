...
ImuProcess::ImuProcess(rclcpp::Node::SharedPtr nh) : Eye3d(M3D::Identity()),
                           Zero3d(0, 0, 0), b_first_frame(true), imu_need_init(true),nh_(nh)
{
  init_iter_num = 1;
...
{
  RCLCPP_WARN(nh_->get_logger(),"Reset ImuProcess");
  mean_acc = V3D(0, 0, -1.0);
  mean_gyr = V3D(0, 0, 0);
  angvel_last = Zero3d;
  imu_need_init = true;
...
void ImuProcess::IMU_init(const MeasureGroup &meas, StatesGroup &state_inout, int &N)
{
  ...
  RCLCPP_INFO(nh_->get_logger(),"IMU Initializing: %.1f %%", double(N) / MAX_INI_COUNT * 100);
  V3D cur_acc, cur_gyr;
...
void ImuProcess::UndistortPcl(LidarMeasureGroup &lidar_meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  case VIO:
    ...
    for (int i = 0; i < v_imu.size() - 1; i++)
    {
      ...
      if (rclcpp::Time(tail->header.stamp).seconds() < prop_beg_time) continue;
      ...
      if (rclcpp::Time(head->header.stamp).seconds() < prop_beg_time)
      {
        // printf("00 \n");
      ...
      {
        // printf("22 \n");
        dt = prop_end_time - rclcpp::Time(head->header.stamp).seconds();
...
void ImuProcess::Process2(LidarMeasureGroup &lidar_meas, StatesGroup &stat, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init)
  {
    ...
    if (init_iter_num > MAX_INI_COUNT)
    {
      ...
      imu_need_init = false;
      RCLCPP_INFO(nh_->get_logger(),"IMU Initials: Gravity: %.4f %.4f %.4f %.4f; acc covarience: "
               ...
               cov_gyr[2]);
      RCLCPP_INFO(nh_->get_logger(),"IMU Initials: ba covarience: %.8f %.8f %.8f; bg covarience: "
               "%.8f %.8f %.8f",
