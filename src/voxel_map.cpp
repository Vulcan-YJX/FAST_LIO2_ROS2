...
void loadVoxelConfig(rclcpp::Node::SharedPtr nh, VoxelMapConfig &voxel_config)
{
  nh->declare_parameter<bool>("publish.pub_plane_en", voxel_config.is_pub_plane_map_);
  nh->get_parameter("publish.pub_plane_en", voxel_config.is_pub_plane_map_);

  nh->declare_parameter<int>("lio/max_layer", voxel_config.max_layer_);
  nh->get_parameter("lio/max_layer", voxel_config.max_layer_);

  nh->declare_parameter<double>("lio/voxel_size", voxel_config.max_voxel_size_);
  nh->get_parameter("lio/voxel_size", voxel_config.max_voxel_size_);

  nh->declare_parameter<double>("lio/min_eigen_value", voxel_config.planner_threshold_);
  nh->get_parameter("lio/min_eigen_value", voxel_config.planner_threshold_);

  nh->declare_parameter<double>("lio/sigma_num", voxel_config.sigma_num_);
  nh->get_parameter("lio/sigma_num", voxel_config.sigma_num_);

  nh->declare_parameter<double>("lio/beam_err", voxel_config.beam_err_);
  nh->get_parameter("lio/beam_err", voxel_config.beam_err_);

  // nh->declare_parameter<vector<int>>("lio/layer_init_num", voxel_config.layer_init_num_);
  // voxel_config.layer_init_num_ = nh->get_parameter("lio/layer_init_num");
  // nh->get_parameter("lio/layer_init_num", voxel_config.layer_init_num_);
  voxel_config.layer_init_num_ = vector<int>{5,5,5,5,5};
  
  nh->declare_parameter<int>("lio/max_points_num", voxel_config.max_points_num_);
  nh->get_parameter("lio/max_points_num", voxel_config.max_points_num_);

  nh->declare_parameter<int>("lio/max_iterations", voxel_config.max_iterations_);
  nh->get_parameter("lio/max_iterations", voxel_config.max_iterations_);

  nh->declare_parameter<bool>("local_map/map_sliding_en", voxel_config.map_sliding_en);
  nh->get_parameter("local_map/map_sliding_en", voxel_config.map_sliding_en);

  nh->declare_parameter<int>("local_map/half_map_size", voxel_config.half_map_size);
  nh->get_parameter("local_map/half_map_size", voxel_config.half_map_size);

  nh->declare_parameter<double>("local_map/sliding_thresh", voxel_config.sliding_thresh);
  nh->get_parameter("local_map/sliding_thresh", voxel_config.sliding_thresh);
...
void VoxelMapManager::StateEstimation(StatesGroup &state_propagat)
{
  ...
  for (int iterCount = 0; iterCount < config_setting_.max_iterations_; iterCount++)
  {
    ...
    if (!EKF_stop_flg && (rematch_num >= 2 || (iterCount == config_setting_.max_iterations_ - 1)))
    {
      ...
      convertEulerToQuaternionMsg(euler_cur,geoQuat_);
...
void VoxelMapManager::pubVoxelMap(const rclcpp::Node::SharedPtr nh)
{
  ...
  for (size_t i = 0; i < pub_plane_list.size(); i++)
  {
    ...
    pubSinglePlane(nh,voxel_plane, "plane", pub_plane_list[i], alpha, plane_rgb);
  }
...
void VoxelMapManager::pubSinglePlane(const rclcpp::Node::SharedPtr nh,visualization_msgs::msg::MarkerArray &plane_pub, const std::string plane_ns, const VoxelPlane &single_plane,
                                     const float alpha, const Eigen::Vector3d rgb)
{
  visualization_msgs::msg::Marker plane;
  plane.header.frame_id = "camera_init";
  plane.header.stamp = nh->now();
  plane.ns = plane_ns;
